...
public class WaitingWordsi extends BaseWordsi {
  ...
  /**
   * The {@link Clustering} implementation to use when all data points have been
   * observed.
   */
  ...
  /**
   * The {@link AssignmentReporter} to use for reporting clustering assignments.
   */
  private final AssignmentReporter reporter;

  /**
   * Creates a new {@link WaitingWordsi}.  The number of clusters is left unset,
   * which requires that the {@link Clustering} algorithm be able to decide on
  ...
  private void clusterTerm(String senseName,
    ...
    String[] contextLabels = contextLabels(senseName);
