 ...
 */
public class SenseEvalContextExtractor implements ContextExtractor {
  ...
  /**
   * Creates a new {@link SenseEvalContextExtractor}.
   *
   ...
   *        compose the context.
   */
  public SenseEvalContextExtractor(ContextGenerator generator,
                                   int windowSize) {
  ...
  public void processDocument(BufferedReader document, Wordsi wordsi) {
    ...
    // It's possible that the SenseEval/SemEval parser failed to find the focus
    // word.  For these cases, skip the context.
    if (!it.hasNext())
      return;
