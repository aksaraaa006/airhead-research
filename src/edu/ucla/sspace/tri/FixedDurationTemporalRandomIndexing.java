 ...
 * duration of a semantic slice is fixed.  Specifically, the following three
 * properties are required: <ol>
 *
 *   <li> A fixed length semantic slice will be used throughout the entire data
 *   processing, as specified by a {@link TimeSpan}.
 *
 *   <li> All documents come in order from least recent to most recent
 *
 *   <li> If a new document's timestamp exceeds the current semenatic slice's
 *   duration (i.e. that it would be a part of a different semantic slice), the
 *   current semantic slice's data may be discarded and a new slice created.
 *
 * </ol>
 *
 ...
 *
 * <dt> <i>Property:</i> <code><b>{@value #SEMANTIC_SLICE_DURATION_PROPERTY}
 *      </b></code> <br>
 *      <i>Default:</i> 1 month
 *
 ...
 *      semantic slices generated by this instance.<p>
 * 
 * </dl><p>
 *
 ...
 * within the semantic slices.  That is, multiple threads may be used to process
 * all of a semantic slices documents, provided that the documents from the next
 * slice are not interleaved.  If the document orderering and time span are
 ...
 *
 * // As threads finish processing a semantic slice, they add the value of the
 * // next time stamp as a key in this map, which allows the processing thread
 * // (see partitionHook below) to determine the start time of the next slice
 ...
 * // after the slice has been finished.  
 ...
 *      // the number of documents in a slice would be less than the number of
 *      // threads, this ensures that thread processing the slice after the next
 *      // correctly waits.
 *      Long ssStart = futureStartTimes.firstKey();
 *      futureStartTimes.clear(); // reset for next slice
 *
 ...
 *
 * // The starting time for the current semantic slice.  This value is used to
 * // determine if processing the next document would cause the current slice
 * // to be partitioned and a new slice created.
 * final AtomicLong startTimeOfCurrentSlice = new AtomicLong();
 *
 * // Before a Thread blocks waiting for slice processing, it enqueues the
 * // time for its next document (exceeding the duration).  These times are used
 * // to select the start time for the next slice.
 ...
 *		    if (docNumber == 1) {
 *			startTimeOfCurrentSlice.set(docTime);
 ...
 *		    // maximum duration of the current slice.  Loop to ensure
 *		    // that if this thread does loop and another thread has an
 *		    // earlier time that exceeds the time period, then this
 *		    // thread will block until the earlier slice has finished
 *		    // processing
 *		    while (!timeSpan.insideRange(startTimeOfCurrentSlice.get(), docTime)) {
 *			try {
 ...
 * documents must be in order according to their semantic slice.  In addition,
 * the first document seen for a semantic slice should be the earliest for that
 * slice.  This behavior is most easily accomplished by sorting the documents
...
public class FixedDurationTemporalRandomIndexing 
     ...
     * specified using the {@value #SEMANTIC_SLICE_DURATION_PROPERTY} property.
     */
    public static final TimeSpan DEFAULT_SEMANTIC_SLICE_DURATION = 
    ...
    /**
     * The property to set duration of a semantic slice using a {@link TimeSpan}
     ...
     *         #SEMANTIC_SLICE_DURATION_PROPERTY} property is not set 
     */
	...
	String timeSpanProp = 
	    props.getProperty(SEMANTIC_SLICE_DURATION_PROPERTY);

	sliceDuration = (timeSpanProp == null) 
	    ? DEFAULT_SEMANTIC_SLICE_DURATION
