...
def answerSATQuestion(SATQuestion, listOfListOfAnalogousPairs):
    ...
    print "searching for answer to %s:%s" % (source)
...
def getScaledSimilarityMeasure(synset1, synset2):
    ...
    return (2.0 * similiarity) / float(normDepth1 + normDepth2)
...
def getAverageDescendantDepth(synset):
    ...
    if synset not in AVG_DESCENDENT_DEPTH_CACHE:
        ...
        depthSum = 0
        leaves = 0
        print "number of immediate hyponyms: %d" % (len(hyponyms))
        ...
        print "depth %f, depthSum %f, leaves %f" % (depth, depthSum, leaves)
        normDepth = depth / (float(depthSum) / float(leaves))
        AVG_DESCENDENT_DEPTH_CACHE[synset] = normDepth
...
def main(argv=None):
    ...
    for question in listOfQuestions:
        answerSATQuestion(question, listOfListOfAnalogousPairs)
