...
public class LSA {
	...
	while ((line = br.readLine()) != null) {
		...
		if (!isValid(cleaned))
		    continue;
		words.add(cleaned);
		wordToDocumentCount.put(new Index(cleaned, filename), 
					Integer.valueOf(
					1 + getCount(cleaned, filename)));
	    }
	}
	br.close();
    }
	...
	while ((line = br.readLine()) != null) {
		...
		if (occurrence.intValue() > 0) {
		    wordToDocumentCount.put(new Index(word, docIter.next()), 
					     occurrence);
		}
	    }
	}
    ...
    public void loadSVDresults(String filename) 
	...
	for (Map.Entry<String,double[]> e : wordToApprox.entrySet()) {
		...
		Double cosDist = Double.valueOf(cosDist(vec, vec2));
