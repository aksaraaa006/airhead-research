 ...
 */ 
public class MultiSenseWordSimilarityEvaluationRunner {
    ...
    public static WordSimilarityReport evaluate(
        ...
        for (WordSimilarity pair : wordPairs) {
            ...
            // get the sense vectors for the pair
            List<Vector> word1senses = 
                getSenseVectors(pair.getFirstWord(), sspace);
            List<Vector> word2senses = 
                getSenseVectors(pair.getSecondWord(), sspace);

            // check that the s-space had both words
            if (word1senses.isEmpty() || word2senses.isEmpty()) {
                unanswerable++;
                continue;
            }

            double closestSim = Double.MIN_VALUE;
            for (Vector v1 : word1senses) {
                for (Vector v2 : word2senses) {
                
                    // use the similarity result and scale it based on the
                    // original answers
                    double similarity = 
                        Similarity.getSimilarity(vectorComparisonType, 
                                                 v1, v1);
                    if (similarity > closestSim)
                        closestSim = similarity;
                }
            }
            double scaled = (closestSim * testRange) +
                test.getLeastSimilarValue();
