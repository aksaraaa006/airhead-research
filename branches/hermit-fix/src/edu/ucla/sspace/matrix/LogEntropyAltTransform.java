 ...
 */
public class LogEntropyAltTransform implements AltTransform {
    ...
    /**
     * Creates an instance of {@code LogEntropyAltTransform} from a {@link
     * Matrix}.
     */
    public LogEntropyAltTransform(Matrix matrix) {
        ...
        if (matrix instanceof SparseMatrix) {
            ...
            for (int row = 0; row < matrix.rows(); ++row) {
                ...
                // Compute the entropy of each row based on the occurances of
                // each row.
                for (int index : nonZeros) {
                    double value = rowVec.get(index);
    ...
    /**
     * Creates an instance of {@code LogEntropyAltTransform} from a {@link
     * File} of format {@link Format}.
     */
    public LogEntropyAltTransform(File inputMatrixFile, Format format) {
        // Get the row sums.
        ...
        iter = MatrixIO.iterate(inputMatrixFile, format);
        while (iter.hasNext()) {
            MatrixEntry entry = iter.next();
