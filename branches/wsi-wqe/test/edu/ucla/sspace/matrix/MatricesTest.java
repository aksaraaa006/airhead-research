...
public class MatricesTest {
    ...
    @Test public void copyDenseTest() {
        double[][] data1 = {{1, 2, 3, 4},
                            {1, 1, 1, 1}, 
                            {5, 4, 3, 2}};
        Matrix original = new ArrayMatrix(data1);
        Matrix copied = Matrices.copy(original);

        assertEquals(original.rows(), copied.rows());
        assertEquals(original.columns(), copied.columns());
        assertNotSame(original, copied);

        for (int row = 0; row < original.rows(); ++row)
            for (int col = 0; col < original.columns(); ++col) 
                assertEquals(original.get(row, col), copied.get(row, col),
                             .000001);
    }

    @Test public void copySparseTest() {
        double[][] data1 = {{1, 2, 0, 4},
                            {1, 0, 1, 1}, 
                            {5, 0, 3, 0}};
        Matrix original = new AtomicGrowingSparseHashMatrix();
        for (int row = 0; row < 4; row++)
            for (int col = 0; col < 3; col++)
                original.set(row, col, data1[row][col]);
        Matrix copied = Matrices.copy(original);
    ...
    @Test public void copyToDenseTest() {
        double[][] data1 = {{1, 2, 3, 4},
                            {1, 1, 1, 1}, 
                            {5, 4, 3, 2}};
        Matrix original = new ArrayMatrix(data1);
        Matrix copied = Matrices.copyTo(original, new YaleSparseMatrix(4, 3));
    ...
    @Test public void copyToSparseTest() {
        double[][] data1 = {{1, 2, 0, 4},
                            {1, 0, 1, 1}, 
                            {5, 0, 3, 0}};
        Matrix original = new YaleSparseMatrix(4, 3);
        for (int row = 0; row < 4; row++)
            for (int col = 0; col < 3; col++)
                original.set(row, col, data1[row][col]);

        Matrix copied = Matrices.copyTo(original, new ArrayMatrix(4, 3));
    ...
    @Test public void multiplyLeftDiagonalTest() {
    ...
    @Test public void multiplyRightDiagonalTest() {
        double[] data1 = {1, 2, 3, 4, 1, 1, 1, 1, 5, 4, 3, 2};
        Matrix leftMatrix = new ArrayMatrix(3, 4, data1);
        double[] data2 = {1, 2, 3, 0};
        Matrix rightMatrix = new DiagonalMatrix(data2);
